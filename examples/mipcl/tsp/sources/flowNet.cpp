...
void CFlowNet<Tflow>::PR_relabel(int v)
{
	...
	for (register int i=m_ipEdgeSep[v]; i < k; i++)
...
void CFlowNet<Tflow>::PR_initSolution()
{
	...
	Tflow uf,LoCap;
	for (register int i=m_ipEdgeSep[m_iSource]; i < k; i++)
...
void CFlowNet<Tflow>::GH_printSolution()
{
	...
	for (register int i=1; i < m_iVertNum; i++) {
...
{
	for (register int v=0; v < m_iVertNum; v++) {
		m_ipPrec[v]=NIL;
		m_ipDepth[v]=0;
		m_ipThread[v]=v;
	}
...
void CFlowNet<Tflow>::printNet()
{
	...
	for (i=0; i < m_iVertNum; i++) {
		fout << i << ": ";
		for (register int j=m_ipEdgeSep[i];j < m_ipEdgeSep[i+1]; j++) {
			e=m_ipEdge[j];
