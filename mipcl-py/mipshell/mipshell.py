...
class Var:
    ...
    Attributes:
        ...
        hd (int): handle;
        type (int): type;
        prev (Var): previous variable in the linked list of problem variables.
...
class LinSum:
    ...
    def __rmul__(self,a):
        """Multiplier the coefficients by ``a`` in all terms of the ``self`` object. 
...
class Ctr:
    ...
    """
    def __init__(self,lsum,lhs=-INF,rhs=INF):
    ...
    def __le__(self,rhs):
        """Change the right hand side of ``self`` constraint.
    ...
    def __ge__(self,lhs):
        """Change the left hand side of ``self`` constraint.
...
class Problem:
    ...
    Attributes:
       ...
       isPureLP (bool): if ``True``, the ;
       is_solution (bool): when the problem is solved, this flag is set to ``True`` if a feasible solution has been found;
    ...
    def addFunc(self,func):
        """Add a function constrsint to the problem list of function constrsints.
    ...
    def mipclModel(self,silent):
        ...
        for j, var in enumerate(self.vars):
            ...
            var.val = 0.0 # is used when adding constraints to handke terms with same variables
            if var.priority:
...
# model function constraints; each such constraint is represented by 3 equations
    ...
    def optimize(self,silent=True,timeLimit=10000000):
        ...
        self.mp.optimize(timeLimit,0.0,self.name)
        ...
        if self.is_solution is not None:
            if self.is_solution == True:
                ...
                if self.isPureLP: # LP
                    ...
                    q = 0.0
                    for ctr in self.ctrs:
                        if abs(ctr.price) > 1.0e-6:
                            if ctr.price > 1.0e-6:
                                print('{:.4f} x ({!s})'.format(ctr.price,ctr._repr(left=False)))
                                q += ctr.rhs * ctr.price
                            else:
                                print('{:.4f} x ({!s})'.format(ctr.price,ctr._repr(right=False)))
                                q += ctr.lhs * ctr.price

                    for var in self.vars:
                        if abs(var.val) > 1.0e-6:
                            if var.val > 1.0e-6:
                                print('{:.4f} x ({!r} <= {:.4f})'.format(var.val,var.name,var.ub))
                                q += var.ub * var.val
                            else:
                                print('{:.4f} x ({:.4f} <= {!r})'.format(var.val,var.lb,var.name))
                                q += var.lb * var.val
                    print('= (0 <= {:.4f})'.format(q))
                else: # MIP
            ...
            with open(solfile,'w') as f:
                ...
                if self.isPureLP: # LP
                    ...
                    for ctr in self.ctrs:
                        if abs(ctr.price) > 1.0e-6:
                            if ctr.price > 1.0e-6:
                                f.write('{:.4f} x ({!s})'.format(ctr.price,ctr._repr(left=False)))
                                q += ctr.rhs * ctr.price
                            else:
                                f.write('{:.4f} x ({!s})'.format(ctr.price,ctr._repr(right=False)))
                                q += ctr.lhs * ctr.price

                    for var in self.vars:
                        if abs(var.val) > 1.0e-6:
                            f.write('{:.4f} x '.format(var.val))
                            if var.val > 1.0e-6:
                                f.write('{:.4f} x ({!r} <= {:.4f})'.format(var.val,var.name,var.ub))
                                q += var.ub * var.val
                            else:
                                f.write('{:.4f} x ({:.4f} <= {!r})'.format(var.val,var.lb,var.name))
                                q += var.lb * var.val
...
def VarVector(sizes, name, type=REAL, lb=0, ub=VAR_INF, priority=0):
